...
import { adminBannerSchema, adminListQuerySchema } from "@/server/admin/validators";
...
export async function GET(request: Request) {
  ...
  const result = await listAdminBanners(parseListQuery(request));
...
export async function POST(request: Request) {
  ...
  const payload = adminBannerSchema.parse(await request.json());
  const requestMeta = await getRequestMeta();
  const banner = await upsertAdminBanner(payload, authenticatedAdmin.session, requestMeta);
  return NextResponse.json({ ok: true, banner }, { status: payload.id ? 200 : 201 });
}
